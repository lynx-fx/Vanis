...
import { Download, File, Clock, AlertCircle, DownloadCloud } from "lucide-react"
...
export default function DownloadClient({ code }) {
  ...
  const handleDownloadAll = async () => {
    if (uploadedFiles.length === 0 || isExpired) return

    setDownloadingAll(true)
    toast.info(`Starting download of ${uploadedFiles.length} files...`)
    ...
    if (successCount > 0 && failCount === 0) {
      ...
      toast.warning(`${successCount} files downloaded, ${failCount} failed`)
    } else {
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 max-w-4xl">
          <div className="text-center mb-6 sm:mb-8 mt-16 sm:mt-20">
            ...
            <p className="text-gray-400 text-xs sm:text-sm md:text-base max-w-xl mx-auto px-4">
              Files hooked to link will be displayed here. Use the download code provided in the URL to access your
              files.
            </p>
          </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                {uploadedFiles.length > 0 && (
                  <button
                    ...
                    disabled={isExpired || downloadingAll}
